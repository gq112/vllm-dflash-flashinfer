...
template <typename scalar_t_in, typename scalar_t_cache, int head_dim,
          bool interleave>
__global__ void dflashKNormRopeMultiCacheUpdateKernel(
    void const* all_k_void, void const* all_v_void, void* key_cache_void,
    void* value_cache_void, void const* k_norm_weights_void,
    void const* cos_sin_cache_void, int64_t const* positions,
    int64_t const* slot_mapping, int const num_layers, int const num_ctx,
    int const num_kv_heads, int64_t const all_k_stride0,
    int64_t const all_k_stride1, int64_t const all_k_stride2,
    int64_t const all_v_stride0, int64_t const all_v_stride1,
    int64_t const all_v_stride2, int64_t const key_cache_stride0,
    int64_t const key_cache_stride1, int64_t const key_cache_stride2,
    int64_t const key_cache_stride3, int64_t const value_cache_stride0,
    int64_t const value_cache_stride1, int64_t const value_cache_stride2,
    int64_t const value_cache_stride3, int const block_size, float const eps,
  ...
  if constexpr ((std::is_same_v<scalar_t_in, c10::BFloat16>) ||
    ...
    int const heads_per_layer = num_ctx * num_kv_heads;
    int const total_heads = num_layers * heads_per_layer;
    if (global_warp_idx >= total_heads) return;

    int const layer_idx = global_warp_idx / heads_per_layer;
    int const rem = global_warp_idx % heads_per_layer;
    int const token_idx = rem / num_kv_heads;
    int const head_idx = rem % num_kv_heads;
    int64_t const slot_idx = slot_mapping[token_idx];
    if (slot_idx < 0) return;
    ...
    int64_t const k_row_offset =
        ...
        static_cast<int64_t>(head_idx) * all_k_stride2;
    int64_t const v_row_offset =
        static_cast<int64_t>(layer_idx) * all_v_stride0 +
        static_cast<int64_t>(token_idx) * all_v_stride1 +
        static_cast<int64_t>(head_idx) * all_v_stride2;
    int64_t const cache_row_offset =
        static_cast<int64_t>(layer_idx) * key_cache_stride0 +
        block_idx * key_cache_stride1 + block_offset * key_cache_stride2 +
        static_cast<int64_t>(head_idx) * key_cache_stride3;
    int64_t const value_cache_row_offset =
        static_cast<int64_t>(layer_idx) * value_cache_stride0 +
        block_idx * value_cache_stride1 +
        block_offset * value_cache_stride2 +
        static_cast<int64_t>(head_idx) * value_cache_stride3;
    int64_t const dim_offset = lane_id * num_elems_per_thread;
...
template <typename scalar_t_in, typename scalar_t_cache>
void launchDFlashKNormRopeMultiCacheUpdate(
    void const* all_k, void const* all_v, void* key_cache, void* value_cache,
    void const* k_norm_weights, void const* cos_sin_cache,
    int64_t const* positions, int64_t const* slot_mapping, int num_layers,
    int num_ctx, int num_kv_heads, int head_dim, int64_t all_k_stride0,
    int64_t all_k_stride1, int64_t all_k_stride2, int64_t all_v_stride0,
    int64_t all_v_stride1, int64_t all_v_stride2,
    int64_t key_cache_stride0, int64_t key_cache_stride1,
    int64_t key_cache_stride2, int64_t key_cache_stride3,
    int64_t value_cache_stride0, int64_t value_cache_stride1,
    int64_t value_cache_stride2, int64_t value_cache_stride3, int block_size,
    int rotary_dim, float eps, bool interleave, cudaStream_t stream) {
  constexpr int block_threads = 256;
  constexpr int warps_per_block = block_threads / 32;
  int const total_warps = num_layers * num_ctx * num_kv_heads;
...
#define LAUNCH_HEAD_DIM(HEAD_DIM)                                          \
  do {                                                                     \
    if (interleave) {                                                      \
      dflashKNormRopeMultiCacheUpdateKernel<scalar_t_in, scalar_t_cache,   \
                                            (HEAD_DIM), true>              \
          <<<grid, block, 0, stream>>>(                                    \
              all_k, all_v, key_cache, value_cache, k_norm_weights,        \
              cos_sin_cache, positions, slot_mapping, num_layers, num_ctx, \
              num_kv_heads, all_k_stride0, all_k_stride1, all_k_stride2,   \
              all_v_stride0, all_v_stride1, all_v_stride2,                 \
              key_cache_stride0, key_cache_stride1, key_cache_stride2,     \
              key_cache_stride3, value_cache_stride0, value_cache_stride1, \
              value_cache_stride2, value_cache_stride3, block_size, eps,   \
              rotary_dim);                                                \
    } else {                                                               \
      dflashKNormRopeMultiCacheUpdateKernel<scalar_t_in, scalar_t_cache,   \
                                            (HEAD_DIM), false>             \
...
void dflash_k_norm_rope_multi_cache_update(
  ...
  STD_TORCH_CHECK(all_k.dim() == 4,
                  "all_k must be [num_layers, num_ctx, num_kv_heads, head_dim]");
  STD_TORCH_CHECK(all_v.dim() == 4 && all_v.size(0) == all_k.size(0) &&
                      all_v.size(1) == all_k.size(1) &&
                      all_v.size(2) == all_k.size(2) &&
                      all_v.size(3) == all_k.size(3),
                  "all_v must have the same shape as all_k");
  STD_TORCH_CHECK(key_cache.dim() == 5 && value_cache.dim() == 5,
                  "key_cache and value_cache must be 5D");
  STD_TORCH_CHECK(k_norm_weights.dim() == 2,
                  "k_norm_weights must be [num_layers, head_dim]");
  STD_TORCH_CHECK(positions.dim() == 1, "positions must be [num_ctx]");
  ...
  STD_TORCH_CHECK(key_cache.size(0) == num_layers &&
                      value_cache.size(0) == num_layers &&
                      key_cache.size(3) == num_kv_heads &&
                      value_cache.size(3) == num_kv_heads &&
                      key_cache.size(4) == head_dim &&
                      value_cache.size(4) == head_dim,
                  "cache shape must be [num_layers, num_blocks, block_size, "
                  "num_kv_heads, head_dim]");
  STD_TORCH_CHECK(key_cache.size(2) == value_cache.size(2),
                  "key_cache and value_cache block sizes must match");
  STD_TORCH_CHECK(key_cache.size(1) == value_cache.size(1),
                  "key_cache and value_cache num_blocks must match");
  STD_TORCH_CHECK(all_k.scalar_type() == all_v.scalar_type() &&
                      ...
                      all_k.scalar_type() == k_norm_weights.scalar_type(),
                  "all K/V/cache tensors and k_norm_weights must share dtype");
  STD_TORCH_CHECK(all_k.stride(3) == 1 && all_k.stride(2) == head_dim,
                  "all_k must have contiguous [num_kv_heads, head_dim] "
                  "inner dimensions");
  STD_TORCH_CHECK(all_v.stride(3) == 1 && all_v.stride(2) == head_dim,
                  "all_v must have contiguous [num_kv_heads, head_dim] "
                  "inner dimensions");
  STD_TORCH_CHECK(key_cache.stride(4) == 1 &&
                      value_cache.stride(4) == 1,
                  "cache head_dim must be contiguous");
  STD_TORCH_CHECK(rotary_dim % 2 == 0, "rotary_dim must be even");
  STD_TORCH_CHECK(rotary_dim <= rope_head_size,
                  "rotary_dim must be <= rope_head_size");
  STD_TORCH_CHECK(rope_head_size == head_dim,
                  "DFlash fused multi-cache update expects rope_head_size "
                  "to match head_dim");
  STD_TORCH_CHECK(head_dim % 64 == 0,
                  "DFlash fused multi-cache update requires head_dim "
                  "divisible by 64");
  STD_TORCH_CHECK(rotary_dim % (head_dim / 32) == 0,
                  "DFlash fused multi-cache update requires rotary_dim to be "
                  "divisible by head_dim / 32");
  ...
  VLLM_STABLE_DISPATCH_HALF_TYPES(
      all_k.scalar_type(), "dflash_k_norm_rope_multi_cache_update_kernel", [&] {
        using k_scalar_t = scalar_t;
        VLLM_STABLE_DISPATCH_FLOATING_TYPES(
            cos_sin_cache.scalar_type(), "dflash_k_norm_rope_cache", [&] {
              using cache_scalar_t = scalar_t;
              vllm::dflash_fused_ops::launchDFlashKNormRopeMultiCacheUpdate<
                  k_scalar_t, cache_scalar_t>(
                  ...
                  static_cast<int>(num_kv_heads), static_cast<int>(head_dim),
                  all_k.stride(0), all_k.stride(1), all_k.stride(2),
                  all_v.stride(0), all_v.stride(1), all_v.stride(2),
                  ...
                  static_cast<int>(key_cache.size(2)),
                  static_cast<int>(rotary_dim), static_cast<float>(eps),
                  !is_neox, stream);
            });
      });
