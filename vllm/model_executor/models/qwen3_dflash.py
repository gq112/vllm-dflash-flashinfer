...
_NATIVE_DFLASH_TRITON_CACHE_DTYPES = {
    "auto",
    "float16",
    "bfloat16",
    "float32",
...
@triton.jit
def _dflash_k_norm_rope_cache_triton_qwen3_4kv_kernel(
    all_k,
    all_v,
    key_cache,
    value_cache,
    ...
    cos_sin_cache,
    slot_mapping,
    stride_k_cache_block: tl.int64,
    stride_k_cache_slot: tl.int64,
    stride_k_cache_head: tl.int64,
    stride_k_cache_dim: tl.int64,
    stride_v_cache_block: tl.int64,
    stride_v_cache_slot: tl.int64,
    stride_v_cache_head: tl.int64,
    stride_v_cache_dim: tl.int64,
    ...
    slot_idx = tl.load(slot_mapping + token_idx).to(tl.int64)
    if slot_idx < 0:
        return
    ...
    src_base = (token_idx * 4 + head_offsets) * 128
    if is_neox:
    ...
    k0 = tl.load(all_k + src_base[:, None] + offset0[None, :]).to(tl.float32)
    k1 = tl.load(all_k + src_base[:, None] + offset1[None, :]).to(tl.float32)
    w0 = tl.load(k_norm_weight + offset0).to(tl.float32)
    w1 = tl.load(k_norm_weight + offset1).to(tl.float32)
    ...
    k_cache_base = (
        block_idx * stride_k_cache_block
        + block_offset * stride_k_cache_slot
        + head_offsets[:, None] * stride_k_cache_head
    )
    tl.store(
        key_cache + k_cache_base + offset0[None, :] * stride_k_cache_dim,
        out0,
    )
    tl.store(
        key_cache + k_cache_base + offset1[None, :] * stride_k_cache_dim,
        out1,
    )

    dim_offsets = tl.arange(0, 128)
    value = tl.load(all_v + src_base[:, None] + dim_offsets[None, :])
    v_cache_base = (
        block_idx * stride_v_cache_block
        + block_offset * stride_v_cache_slot
        + head_offsets[:, None] * stride_v_cache_head
    )
    tl.store(
        value_cache + v_cache_base + dim_offsets[None, :] * stride_v_cache_dim,
        value,
    )


def _dflash_k_norm_rope_cache_triton_qwen3_4kv(
    all_k: torch.Tensor,
    all_v: torch.Tensor,
    ...
    num_ctx, num_kv_heads, head_dim = all_k.shape
    ...
    _dflash_k_norm_rope_cache_triton_qwen3_4kv_kernel[(num_ctx,)](
        all_k,
        all_v,
        key_cache,
        value_cache,
        ...
        if can_write_triton_cache:
            ...
            for attn in self._attn_layers:
                ...
                supports_cache = (
                    ...
                    and kv_cache_dtype in _NATIVE_DFLASH_TRITON_CACHE_DTYPES
                )
                if supports_cache:
